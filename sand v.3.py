...
# ==========================
# Расчёт геометрии и массы с опциональным ограничением по высоте
# ==========================
def compute(L, W, phi_deg, rho, H_max_limit=None):
    m = 1.0 / math.tan(math.radians(phi_deg))
    Hmax = min(L/(2*m), W/(2*m))
    if H_max_limit is not None:
        Hmax = min(Hmax, H_max_limit)  # ограничиваем высоту
...
# ==========================
# Генерация отчёта с визуализацией усечённой вершины
# ==========================
def generate_sand_sites_report_truncated(sites_params, doc_path):
    doc = Document()
    ...
    for idx, params in enumerate(sites_params, start=1):
        ...
        rho = params["rho"]
        name = params["name"]
        H_max_limit = params.get("H_max_limit", None)  # опциональное ограничение

        res = compute(L,W,phi,rho,H_max_limit=H_max_limit)
        m = res["m"]
        H = res["Hmax"]
        Ltop = res["Ltop"]
        Wtop = res["Wtop"]
        ...
        # Верхний усечённый участок
        if H_max_limit is not None and Atop > 0:
            left = (L - Ltop)/2.0
            bottom = (W - Wtop)/2.0
            top_rect = Rectangle((left, bottom), Ltop, Wtop, fill=False, linestyle='--', color='red')
            ax.add_patch(top_rect)
            ax.text(left + Ltop/2.0 - 1, bottom + Wtop/2.0, f"Truncated Apex", fontsize=9, color='red')
        ...
        # Линия усечения
        if H_max_limit is not None:
            ax.hlines(H_max_limit, 0, L, colors='red', linestyles='--', label='H_max_limit')
            ax.text(L+0.2, H_max_limit, f"H_max={H_max_limit} м", color='red', fontsize=9)
...
# ==========================
sites_params = [
    {"L":30.0, "W":30.0, "phi":32.0, "rho":1.60, "name":"Площадка 30×30, Сухой песок", "H_max_limit":6.0}, # Максимальная высота опциональна
    {"L":30.0, "W":30.0, "phi":28.0, "rho":1.85, "name":"Площадка 30×30, Влажный песок", "H_max_limit":6.0},
    {"L":50.0, "W":50.0, "phi":32.0, "rho":1.60, "name":"Площадка 50×50, Сухой песок", "H_max_limit":6.0},
    {"L":50.0, "W":50.0, "phi":28.0, "rho":1.85, "name":"Площадка 50×50, Влажный песок", "H_max_limit":6.0},
]

doc_path = "Площадки_расчёт_усечённая_пирамида_визуально.docx"
generate_sand_sites_report_truncated(sites_params, doc_path)
